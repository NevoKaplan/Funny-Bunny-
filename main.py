...
class Bunny:
    ...
    def furthest_bunny(self):
        max = -1
        index = -1
        for t in range(len(self.loc_list)):
            if self.loc_list[t][1] >= max:
                max = self.loc_list[t][1]
                index = t
        return index
    ...
    while game:
        for i in range(players_num):
            if player_list[i] is not None:
                ...
                count = 0
                while event == -1 and (player_list[i].loc_list[bunny_num][1] + hop == board.get_hole() or board.board[
                    player_list[i].loc_list[bunny_num][1] + hop] == "occ" or not board.is_location_valid(
                        player_list[i].loc_list[bunny_num][1], player_list[i].loc_list[bunny_num][1] + hop)) and \
                        player_list[i].next_bunny(bunny_num) != -2:
                    bunny_num = player_list[i].next_bunny(bunny_num)
                    count += 1
                    if count == 4:
                        hop, event = random_card()
                        count = 0
                if event == -1:
                    ...
                    if player_list[i].loc_list[bunny_num][1] == board.get_hole():
                        ...
                        print(player_list[i].name + "'s bunny number " + str(bunny_num + 1) + " " + random.choice(
                            ["burned in a fire. He's dead now", "was sent to the abyss.",
                             "is an asshole. He's with god now.", "is a loser. He's out of the game.",
                             "is in a better place now. Maybe", "was removed from the game.", "dead, sorry."]))
                        if player_list[i].get_alive() == -2:
                    ...
                    if board.board[hop] == "occ":
                        ...
                        print(player_list[q].name + "'s bunny number " + str(player_list[q].loc_list[w][0]) + " " + random.choice(
                            ["burned in a fire. He's dead now", "was sent to the abyss.",
                             "is an asshole. He's with god now.", "is a loser. He's out of the game.",
                             "is in a better place now. Maybe", "was removed from the game.", "dead, sorry."]))
